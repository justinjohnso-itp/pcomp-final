...
function handleMIDIMessage(message) {
  ...
  if (status === 176) {
    ...
    if (data1 === 10) {
      state.dist1a = data2 / 127;
    }
    if (data1 === 11) {
      state.dist1b = data2 / 127;
    }
    if (data1 === 12) {
      state.dist2a = data2 / 127;
    }
    if (data1 === 13) {
      state.dist2b = data2 / 127;
    }
    if (data1 === 14) {
      state.pot1 = data2 / 127;
    }
    if (data1 === 15) {
...
let app = {
  async initScene() {
    ...
    this.heightmapVariable.material.uniforms["waveheightMultiplier"] = {
      value: 0.3,
    };
    this.heightmapVariable.material.defines.GEOM_WIDTH = GEOM_WIDTH.toFixed(1);
    this.heightmapVariable.material.defines.GEOM_HEIGHT =
    ...
    this.smoothShader = this.gpuCompute.createShaderMaterial(SmoothFragment, {
    ...
    gui.add(params, "waveHeight", 0.1, 2.0, 0.05).onChange((newVal) => {
      this.heightmapVariable.material.uniforms["waveheightMultiplier"].value =
        newVal;
    });
    const buttonSmooth = {
  ...
  smoothWater() {
    ...
    const alternateRenderTarget = this.gpuCompute.getAlternateRenderTarget(
      this.heightmapVariable
    );

    for (let i = 0; i < 10; i++) {
      this.smoothShader.uniforms["smoothTexture"].value =
